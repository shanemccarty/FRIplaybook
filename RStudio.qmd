...
``` r
setwd("/Users/allisonanemone/FRI")
read.csv("/Users/allisonanemone/Downloads/alldata_vaccinations.csv")
```

This code only works on that one computer. It will fail on your teammates' computers and on your instructor's computer, because the folder `/Users/allisonanemone/` does not exist there. With a project and file names only, the same code runs for everyone who has the project folder.
:::
...
- **Could not find function** → This error occurs when a function is not recognized. This is often because a package has not been loaded. Make sure you have run `library(package_name)` before using functions from that package.
- **There is no package called…** → This means the package has not been installed yet. Run `install.packages("package_name")` to install it, then `library(package_name)` to load it.
