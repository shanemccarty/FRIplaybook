...
Use the .clean datafile: `r current_date`.data.team1.clean.xlsx . Notice that the file path has "data/`r current_date`.data.team1.clean.xlsx" because the folder (data) is holding the Posit Cloud file (`r current_date`.data.team1.clean.xlsx). You do NOT need to put your file inside of a "data" folder. After you uupload your data, you can use the file name only as the path.
...
```{r}
# View first 10 rows
head(healthbeliefsdata, 10)
```

```{r}
# View column names, which are the VARIABLES
names(healthbeliefsdata)
```

```{r}
# Get a summary of all variables
summary(healthbeliefsdata)
```
...
safetydata2 <- read_excel(
  path = "data/10.20.2025.TestData.Team1.xlsx",
  sheet = 1,
  n_max = 1,
  col_names = FALSE
)
