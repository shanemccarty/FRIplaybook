...
|----|----|----|----|
| Import | readxl | `read_excel()` to import your `.xlsx` survey data | Import Data Once, Start a Report, Pre/Post Data, Creating Composites |
| Import | haven | `read_sav()` to import SPSS `.sav` files | Import Data Once, Mutate Variables, Visualize Pre/Post Data |
| Tidy | tidyr\* | `pivot_longer()` and `pivot_wider()` to reshape long ↔ wide data | Pre/Post Data, Visualize Pre/Post Data |
...
| Visualize | ggplot2\* | all plots | Coding a Plot, Visualize a Comparison, Visualize a Relationship, Visualize Pre/Post Data |
...
In your RStudio Project, create a new file that is "R Script" and title it `install.R`. Run the whole file **one time** by clicking the "Source" button. It may take several minutes. If it stops with an error, run the file a few lines at a time to find the package that failed (see [R Software](RStudio.qmd), Running code). You do not need to run it again unless you get a new computer or need a new package.
...
## Loading libraries in your report {#sec-load-block}
...
::: callout-tip
## Go: Do I have to rerun this chunk every time I open my report?

Yes and no, depending on what you are doing.

-   **Rendering: no.** Render starts a fresh R session and runs your whole file from the first line to the last, so it runs the load-library chunk itself. As long as the chunk is at the top of your report, rendering will never fail because a package was not loaded.
-   **Working on your report between renders: yes.** When you open RStudio, its memory is empty, so a chunk in the middle of your report (say, a plot) will fail until the chunks above it have run. That is why [Ref's kickoff](find-the-ref.qmd) has you run the load-library chunk (step 4) and then **Run All Chunks Above** (step 5): the gray ▼ with a line under it, next to the green arrow on the chunk, or **Cmd + Option + P** (Mac) / **Ctrl + Alt + P** (Windows). This runs the load-library chunk, the import chunk, and everything else above the chunk you are on, in order.

This is also why we ask you not to save your workspace (Chapter 3): starting empty every time is what makes the render match what you see while you work.
:::
