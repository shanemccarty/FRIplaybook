...
export_date <- "10.19.2026"   # this year's export date; change it here and every file name in the chapter updates
```
...
- [ ] Open the document in Microsoft Word (not google docs) to confirm it works. Then, save it as `09.28.2026.Lastname.Survey.docx`, or change the date to the current date.
...
- [ ] **Rename `.xlsx` file** using the date of the export in this format: MONTH.DAY.YEAR.data.team#.filetype. For example, if the date is `r export_date` then the correct format of the files should be: `r export_date`.data.team1.xlsx

- [ ] **Make a copy** of the `r export_date`.data.team1.xlsx and rename it to: `r export_date`.data.team1.clean.xlsx.

- [ ] In google sheets, **open up the file with .clean** added to the original file (i.e., `r export_date`.data.team1.clean.xlsx) and **delete the second row (with labels and questions/items)**. You'll notice that the dataset has two headers for variables and labels. You need to delete Row 2 to eliminate those labels.

- [ ] Go to File \> **Download** \> Microsoft Excel.

Once you have completed these steps, you can move to the next chapter to import your data (`r export_date`.data.team1.clean.xlsx) to R or NVivo.

::: callout-tip
## Tips for Data Files

**Rename files** using the date of the export, following MONTH.DAY.YEAR.data.team#.filetype. For example, if you export on `r export_date` then the correct format of the files should be: `r export_date`.data.team1.xlsx. Then, `r export_date`.data.team1.xlsx should be changed to `r current_date`.data.team1.clean.xlsx after removing the second row.

It is important to include the date in your file names because you may download data at different stages of the recruitment process (e.g., after a brief pilot phase, halfway, and at the end). For example, a team that keeps collecting data after `r export_date` will export again at the end, and that later date goes in the file name of the final dataset (for example, 11.06.2026.data.team2.xlsx).
:::
...
- `r export_date`.data.team2.xlsx
- `r export_date`.data.team2.clean.xlsx
:::
...
- `r export_date`.data.team5.xlsx
- `r export_date`.data.team5.clean.xlsx
:::
