...
-   [ ] At the top, go to Data & Analysis tab (top navigation bar)
    -   [ ] Ensure that "Data" is selected (from the second navigation bar)
...
-   [ ] **Rename both files** using the date of the export in this format: MONTH.DAY.YEAR.data.team#.filetype. For example, if the date is `r current_date` then the correct format of the files should be: `r current_date`.data.team1.csv and `r current_date`.data.team1.xlsx

-   [ ] **Make a copy** of the `10.20.2025.data.team1.xlsx` and rename it to: `10.20.2025.data.team1.clean.xlsx.`

-   [ ] In google sheets, **open up the file with .clean** added to the original file (i.e., `10.20.2025.data.team1.clean.xlsx`) and **delete the second row**. You'll notice that the dataset has two headers for variables and labels. You need to delete Row 2 to eliminate the labels.
...
Reminder: Some teams will export datasets at different times during their recruitment process in order to complete the individual and team deadlines. You should export and **Rename both files** using the date of the export, following MONTH.DAY.YEAR.data.team#.filetype. For example, if you export on `r current_date` then the correct format of the files should be: `r current_date`.data.team1.csv and `r current_date`.data.team1.xlsx. Then, `r current_date`.data.team1.xlsx should be changed to `r current_date`.data.team1.clean.xlsx after removing the second row.
